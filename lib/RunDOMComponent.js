...
function processQueue(inst, updateQueue) {
  	for(let i = 0; i < updateQueue.length; i++) {
  		...
  		switch (update.type) {
  			case 'INSERT_MARKUP':
  				DOMTree.insertTreeBefore(inst._hostNode, update.content, getNodeAfter(inst._hostNode, update.afterNode))
  				break;
  			case 'MOVE_EXISTING':
  				moveChild(inst, update.fromNode, getNodeAfter(inst._hostNode, update.afterNode))
  				break;
  			case 'SET_MARKUP':
  				inst._hostNode.innerHTML = update.content
  				break;
  			case 'TEXT_CONTENT':
...
export default class RunDomComponent {
	...
	updateChildren(prevChildren, nextChildren, mountImages, removedNodes) {
    	...
    	for(name in nextChildren) {
    		...
    		if (prevChild != null) {
		        ...
		        let nextChildInstance = instanceComponent(nextElement)
		        nextChildren[name] = nextChildInstance
		        let nextChildMountImage = nextChildInstance.mountComponent(this)
	...
	_mountChildAtIndex(child, mountImage, afterNode, index) {
		child._mountIndex = index
		return this.makeInsertMarkup(child, afterNode, mountImage)
	}
